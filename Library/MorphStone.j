library MorphStone initializer init
    ...
    private function Main takes nothing returns nothing
        ...
        loop
        exitwhen CompareRect[i] == null
            if GetTriggeringRegion() == Rects[i] and RectState[i] == false and j >= 1 and j <= PLAYER_MAXINUM and BackGroundUnits[j] != GetTriggerUnit() and OrangeMushroomSkin[j] != GetTriggerUnit() and OrangeMushroomFloorSkin[j] != GetTriggerUnit() and GetUnitTypeId(GetTriggerUnit()) != GetUnitTypeId(MorphUnit[i]) then
