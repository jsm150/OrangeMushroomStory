library TrueEnding initializer init needs Cinematic
    ...
    private function CMTTick takes nothing returns nothing
        ...
        elseif tk.data == 5 then
            ...
            set PinkMushroom = CreateUnit(Player(11), 'ucry', 15232, -19616, 270 )
        ...
        elseif tk.data == 15 then
            set BlackUnit[1] = CreateUnit(Player(11), 'hgry', 13504, -19328, 270 )
        ...
        elseif tk.data == 16 then
            ...
            set i = i + 1
            endloop
            call PanCameraToTimed(13632, -19328, 3)
        ...
        elseif tk.data == 20 then
            ...
            exitwhen i > PersonPlayer()-2
                if i == 1 then
                    set BlackUnit[i+1] = CreateUnit(Player(11), 'hgry', 12928, -19328, 270 )
                elseif i == 2 then
                    set BlackUnit[i+1] = CreateUnit(Player(11), 'hgry', 13248, -18688, 270 )
                elseif i == 3 then
                    set BlackUnit[i+1] = CreateUnit(Player(11), 'hgry', 14080, -18688, 270 )
                elseif i == 4 then
                    set BlackUnit[i+1] = CreateUnit(Player(11), 'hgry', 13312, -20032, 270 )
                elseif i == 5 then
                    set BlackUnit[i+1] = CreateUnit(Player(11), 'hgry', 13952, -20032, 270 )
                endif
            set i = i + 1
            endloop
        ...
        elseif tk.data == 22 then
            call PanCameraToTimed(15232, -19616, 1)
        ...
        elseif tk.data == 24 then
            ...
            set i = i + 1
            endloop
            call PanCameraToTimed(13504, -19328, 1.5)
        ...
        elseif tk.data == 37 then
            ...
            call PanCameraToTimed(16128, -19424, 1)
        ...
        elseif tk.data == 41 then
            call CameraSetSourceNoise(80, 100000)
            call SetDoodadAnimation(64, -19328, 128.00, 'IOpr', false, "Spell", false)
            ...
            exitwhen i > PLAYER_MAXINUM
                if GetPlayerSlotState(Player(i-1)) == PLAYER_SLOT_STATE_PLAYING then
                    call SetUnitPosition( OrangeMushroom[i], GetRectMinX(gg_rct_EndingStartRect)+(128*(i-1)), GetRectCenterY(gg_rct_EndingStartRect) )
                    call SetUnitPosition( BackGroundUnits[i], GetRectMinX(gg_rct_EndingStartRect)+(128*(i-1)), GetRectCenterY(gg_rct_EndingStartRect) )
                endif
            set i = i + 1
            endloop
            call PanCameraToTimed(384, -19328, 0)
            call StopSound( gg_snd_OM_GayBar, false, true )
    ...
    private function CMTTick2 takes nothing returns nothing
        ...
        elseif tk.data == 7 then
            set PinkMushroom = CreateUnit(Player(11), 'ucry', 192, -19648, 270 )
            call SetUnitBlendTime(PinkMushroom, 0.00)
            call SetUnitAnimation( PinkMushroom, "Stand Upgrade Second" )
            loop
            exitwhen i > PersonPlayer()-1
                if i == 1 then
                    set BlackUnit[i] = CreateUnit(Player(11), 'hgry', -320, -19456, 270 )
                elseif i == 2 then
                    set BlackUnit[i] = CreateUnit(Player(11), 'hgry', 128, -18944, 270 )
                elseif i == 3 then
                    set BlackUnit[i] = CreateUnit(Player(11), 'hgry', 256, -20224, 270 )
                elseif i == 4 then
                    set BlackUnit[i] = CreateUnit(Player(11), 'hmpr', 1024, -18944, 270 )
                elseif i == 5 then
                    set BlackUnit[i] = CreateUnit(Player(11), 'hmpr', 1024, -20224, 270 )
                elseif i == 6 then
                    set BlackUnit[i] = CreateUnit(Player(11), 'hmpr', 1536, -19456, 270 )
                endif
            set i = i + 1
            endloop
            call StartSound(gg_snd_OM_GayBar)
        ...
        elseif tk.data == 9 then
            ...
            call PanCameraToTimed(-320, -19456, 1)
            call MsgPrint(BLACK_NAME + ": No no... 눈 앞에 애피타이저를 그냥 지나칠 순 없잖아?♂")
            if Status.Portal == 0 then
                set tk.data = 12
            endif
        elseif tk.data == 10 then
            call PanCameraToTimed(384, -19328, 1)
        ...
        elseif tk.data == 12 then
            ...
            call SetCameraBounds(384, -19328, 384, -19328, 384, -19328, 384, -19328)
        elseif tk.data == 13 then
        ...
        elseif tk.data == 60 then
            call StartSound(gg_snd_OM_EndingSound2)
        ...
        elseif tk.data == 64 then
            if Status.Portal == 0 then
                call EndingMsgPrint("PS: 다 같이 불행해지는 길을 택하다니... 엄청난 단결력이네요!")
            else
                call EndingMsgPrint("PS: 동료를 배신한 졸렬한 플레이어: " + EscapersName())
            endif
        ...
        elseif tk.data == 66 then
            ...
            exitwhen i > PLAYER_MAXINUM
                if GetPlayerSlotState(Player(i-1)) == PLAYER_SLOT_STATE_PLAYING then
                    call DisplayTimedTextToPlayer(Player(i - 1), 0, 0, 60, "　　　　　　" + TeamColor[i] + GetPlayerName(Player(i-1)) + "|r 님의 세 번째 비밀 코드: " + WorldKey_Code3[i] )
                    call DisplayTimedTextToPlayer(Player(i - 1), 0, 0, 60, "　　　　　　다음 게임부터 \"-code " + WorldKey_Code3[i] + "\"를 입력하시면 캐릭터 외형을 바꿀 수 있습니다.")
                    call JNObjectCharacterSetInt(StringCase(GetPlayerName(Player(i - 1)), false), "Valentine", User_UserList[i - 1].ClearList.Valentine + 1)
            ...
            exitwhen i > PLAYER_MAXINUM
                if GetPlayerSlotState(Player(i-1)) == PLAYER_SLOT_STATE_PLAYING and GetLocalPlayer() == Player(i-1) then
                    call BJDebugMsg("　　　　　　" + TeamColor[i] + GetPlayerName(Player(i-1)) + "|r 님의 세 번째 비밀 코드: " + WorldKey_Code3[i] )
                    call BJDebugMsg("　　　　　　다음 게임부터 \"-code " + WorldKey_Code3[i] + "\"를 입력하시면 캐릭터 외형을 바꿀 수 있습니다.")
                endif
            set i = i + 1
            endloop
            return
    ...
    private function init takes nothing returns nothing
        set ENDING_RECT = gg_rct_TrueEndingRect
